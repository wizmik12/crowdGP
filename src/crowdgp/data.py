...
:class:`CrowdBatch` produced here, so the guarantees this file makes are the
...
:meth:`CrowdLabels.as_ragged` solves this by re-expressing the annotations as
...
class CrowdLabels:
    ...
    Note:
        Everything in this class is NumPy except :meth:`as_ragged` and
        :meth:`gather_batch`. Preprocessing runs once, outside the training
    ...
    def gather_batch(self, X: tf.Tensor, item_global: tf.Tensor) -> CrowdBatch:
        """Assembles an aligned :class:`CrowdBatch` for the given items.
    ...
    def full_batch(self, X: tf.Tensor) -> CrowdBatch:
        """Returns a :class:`CrowdBatch` covering the entire dataset.
