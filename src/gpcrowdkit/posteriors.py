...
[FreeCategoricalZ][gpcrowdkit.posteriors.FreeCategoricalZ] reproduces exactly that.
...
class PosteriorZ(gpflow.Module, abc.ABC):
    ...
    Note:
        [gamma][gpcrowdkit.posteriors.PosteriorZ.gamma] receives both evidence tensors even though the free
...
class FreeCategoricalZ(PosteriorZ):
    ...
    def __init__(
        ...
        Args:
            ...
            init_probs: Optional ``[N, C]`` starting probabilities, normally
                [empirical_class_probs][gpcrowdkit.data.CrowdLabels.empirical_class_probs], which
    ...
    def gamma(self, batch, gp_log, crowd_log):
        """See [PosteriorZ.gamma][gpcrowdkit.posteriors.PosteriorZ.gamma]. Uses only ``batch.item_global``.
